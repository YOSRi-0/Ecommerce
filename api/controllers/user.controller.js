...
exports.login = (req, res) => {
  ...
  User.login(enteredUsername, (err, data) => {
    if (err) {
      ...
      if (enteredPassword !== originPassword) {
        return res.status(401).json({
...
exports.create = (req, res) => {
  ...
  User.create(user, (err, data) => {
    if (err) {
      res.status(500).json({
        message: err.message || "some error occurred whime creating the user",
      });
    } else {
...
exports.deleteAll = (req, res) => {
  User.removeAll((err, data) => {
    if (err)
      res.status(500).json({
        message: err.message || "Some error occured while remove all users",
      });
    else
      res.json({
